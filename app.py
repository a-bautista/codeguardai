...
def analyze_code():
    ...
    if request.method == 'POST':
        ...
        system_message = "You are a code analysis assistant. Analyze the following code for potential improvements or bugs. You must categorize the bugs or errors from a scale of 1 to 3, where 1 indicates an error that MUST be fixed to allow the program to be functional while 2 indicates a potential bug in the code that should be solved and a category 3 indicates a problem related to non optimal code that could be improved upon. For level 1 errors, suggest a solution that keeps the code original functionality."
